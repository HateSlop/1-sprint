...
> 교차 인코더와 바이 인코더를 결합해 더 정확하면서도 빠르게 검색한다.
> 바이 인코더를 사용해 대규모의 문서에서 검색 쿼리와 유사한 소수의 문서를 선별하고, 선별한 소수의 문서는 유사도를 더 정확히 계산할 수 있는 교차 인코더를 사용해 재정렬한다.

- 검색 성능을 높이기 위한 방법
    1. 바이 인코더를 추가 학습시킨다.
    > 문장 임베딩 모델을 사용하려는 데이터셋으로 추가 학습시킨다.
    2. 교차 인코더를 추가한다.
    > 검색 쿼리 문장과 검색 대상 문장을 함께 입력으로 받는 교차 인코더를 추가해 소수의 후보군에서 더 관련성이 높은 문서를 상위에 올린다.

## 11.2 언어 모델을 임베딩 모델로 만들기
> sentence-transformers 라이브러리를 사용한다.
> 문장 임베딩 모델은 **대량의 텍스트 데이터로 사전 학습한 언어 모델 층**과 **풀링층**, 입력 문장의 길이에 따라 달라질 수 있는 출력 차원을 고정된 차원으로 맞추는 역할,으로 나뉜다.
### 11.2.1 대조 학습
- 대조 학습: 관련이 있거나 유사한 데이터는 더 가까워지도록 만들고 관련이 없거나 유사하지 않은 데이터는 더 멀어지도록 하는 학습 방식
### 11.2.2 실습: 학습 준비하기
- 사전 학습된 언어 모델을 불러와 문장 임베딩 모델을 만든다.
- 데이터 전처리를 수행한다.
    - 학습 데이터의 일부를 검증을 위한 데이터셋으로 분리한다.
    - 유사도 점수를 0-1 사이로 정규화한다.
    - torch.utils.data.DataLoader를 사용해 배치 데이터로 만든다.
> 언어 모델을 그대로 사용할 경우 문장 임베딩 모델로서의 역할을 잘 하지 못한다.
### 11.2.3 실습: 유사한 문장 데이터로 임베딩 모델 학습하기
> 준비한 학습 데이터셋으로 언어 모델을 문장 임베딩 모델로 만든다.
- 학습 진행 시간과 언어 모델을 지정한다.
- 손실함수 CosineSimilarityLoss를 사용해 학습 데이터를 문장 임베딩으로 변환하고 두 문장 사이의 유사도를 비교해 학습을 수행한다.
- fit() 메서드에 앞서 준비한 학습 데이터셋, 학습 손실 함수 등 필요한 인자를 전달하면 학습이 진행된다.

## 11.3 임베딩 모델 미세 조정하기
> 임베딩 모델을 추가 학습시켜, 실습 데이터의 문장 사이의 유사도를 더 잘 계산하도록 한다.
### 11.3.1 실습: 학습 준비
- 기본 임베딩 모델을 불러온다.
- 데이터 전처리를 수행한다.
- 유사도를 잘 계산하는지 평가하기 위해 질문과 관련이 없는 기사를 컬럼에 추가한 후, 성능 평가에 사용할 데이터를 최종적으로 생성한다.
### 11.3.2 MNR 손실을 활용해 미세 조정하기
- MNR 손실: 데이터셋에 서로 관련이 있는 문장만 있는 경우 사용하기 좋은 손실 함수다. 하나의 배치 데이터 안에서 다른 데이터의 기사 본문을 관련이 없는 데이터로 사용해 모델을 학습시킨다.
- 중복 데이터를 제거한 후 MNR 손실 함수를 불러온다.
- 미세 조정
    1. 학습 수행 에포크를 지정하고, 모델 학습 이후 지정 경로에 모델을 저장한다.
    2. fit() 메서드에 학습 데이터, 손실 함수, 학습 에포크 수 등을 입력으로 전달해 학습을 진행한다.

## 11.4 검색 품질을 높이는 순위 재정렬
> 교차 인코더와 바이 인코더 두 모델의 장단점을 보완하기 위해 먼저 바이 인코더로 후보군을 압축하고 후보군 내에서 교차 인코더로 순위를 재정렬하는 방법을 사용한다.
> 교차 인코더는 문장의 관련성을 잘 계산하지 못하므로 미세 조정이 필요하다.
- 교차 인코더 학습 데이터셋을 준비한다.
- 교차 인코더의 학습을 수행한다.
    1. 배치 크기와 학습 에포크 수를 설정한다.
    2. 모델 학습 후 모델을 저장할 경로를 지정하고, 학습 데이터셋은 학습 데이터로더로 만든다.
    3. fit() 메서드에 학습 데이터와 설정값을 전달해 학습을 수행한다.

## 11.5 바이 인코더와 교차 인코더로 개선된 RAG 구현하기
> 바이 인코더와 교차 인코더를 조합해 더 개선된 검색 증강 생성을 구현하고 각각의 성능과 시간을 비교해 본다.
1. 기본 임베딩 모델
2. 미세 조정한 임베딩 모델
3. 미세 조정한 임베딩 모델과 교차 인코더 조합

- 평가를 위한 데이터셋을 불러와 선별한다.
- 임베딩을 저장하고 검색하는 함수를 구현한다.
- 교차 인코더를 활용한 순위 재정렬 함수를 정의한다.
    1. 질문의 인덱스는 고정한 상태로 기사 본문 리스트를 순회하면서 모든 질문-내용 쌍을 생성한다.
    2. 교차 인코더에 질문-내용 쌍을 입력하고 유사도 점수 계산 결과에 따라 순위를 재정렬하는 함수를 정의한다.
- 성능 지표와 평가에 걸린 시간을 반환하는 함수를 정의한다.
    1. 각 질문별로 k개의 유사 문서를 검색한다.
    2. 검색 결과 내에 정답 데이터가 포함된 경우 정답을 맞췄다고 계산한다.
    3. 맞춘 데이터의 수와 걸린 시간을 결과로 반환한다.
### 11.5.1 기본 임베딩 모델로 검색하기
1. 문장 임베딩 모델을 불러온다.
2. 불러온 기본 임베딩 모델과 평가 데이터셋의 context 칼럼을 make_embedding_index 함수의 입력으로 넣어 벡터 검색에 사용할 인덱스를 만든다.
3. 평가 데이터셋, 기본 임베딩 모델, 인덱스를 evaluate_hit_rate 함수의 입력으로 넣어 성능과 평가에 걸린 시간을 확인한다.
### 11.5.2 미세 조정한 임베딩 모델로 검색하기
> 사용하는 임베딩 모델이 MRC 데이터로 미세 조정했다는 점 외에는 위와 동일하다.
### 11.5.3 미세 조정한 임베딩 모델과 교차 인코더 조합하기
- 임베딩 모델의 상위 N개 결과를 받아서 교차 인코더가 순위를 유사도순으로 재정렬한 후 상위 K개를 추출하면 임베딩 모델을 통해 유사도가 높은 상위 K개를 바로 뽑았을 때보다 성능을 높일 수 있다.
- 교차 인코더의 경우 속도가 느리기 때문에 전체 문서가 아닌 상위 N개만을 대상으로 계산하도록 범위를 좁힌다. 