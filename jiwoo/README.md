...
> 추론 과정
- 사전 계산 단계: 입력 프롬프트를 처리하는 단계
  - 디코딩 할 때마다 반복적으로 이 단계를 거쳐야 함
  - 동일한 연산을 반복적으로 수행하므로 **비효율적**
- 디코딩: 한 토큰씩 생성하는 단계
  - 동시에 병렬적으로 처리 가능
  - 입력 프롬프트의 길이에 영향 받지 않음

### 7.1.2 중복 연산을 줄이는 KV 캐시

**KV 캐시**: 셀프 어텐션 연산 과정에서 먼저 계산했던 키와 값 결과를 저장해두는 메모리

KV 캐시 메모리 = (데이터 타입에 해당하는 바이트 수) X 2 X (레이어 수) X (토큰 임베딩 차원) X (최대 시퀀스 길이) X (배치 크기)

- 한정된 GPU 메모리 크기로 인하여, 배치 크기가 제한되는 문제 발생
...
- 여러 스트리밍 멀티프로세서(SM)
  ...
  - SRAM
    - 메모리의 크기가 작음
    - 큰 데이터는 HBM으로 이동시켜 저장해야 함
- HBM
  - 메모리의 크기가 큼
 
연산 시간 = ((데이터 타입에 해당하는 바이트 수) X (모델 파라미터가 차지하는 메모리) X **(배치 크기)**) / (하드웨어 연산 속도)

모델 파라미터 이동 시간 = (모델 파라미터가 차지하는 메모리) / (메모리 대역폭)

**최적의 배치 크기**: 연산 시간 = 모델 파라미터의 이동 시간 일 때이므로 (하드웨터 연산 속도) / ((데이터 타입에 해당하는 바이트 수) X (메모리 대역폭))

GPU를 효율적으로 활용하기 위해서는 최대 배치 크기가 최적의 배치 크기만큼 커져야 함
...
> 멀티 헤드 어텐션의 장단점
- 장점
  - 쿼리와 키 사이에 다양한 측면의 관련성 반영 가능
  - 성능 상승
- 단점
  - 헤드의 갯수만큼의 키와 값 벡터를 저장하므로 KV 캐시가 사용해야 할 메모리 증가
  - 계산 속도 저하
 
**멀티 쿼리 어텐션**: 모든 쿼리 벡터가 하나의 키와 값 벡터를 공유
> 멀티 쿼리 어텐션의 장단점
- 장점: 1개의 키와 값 벡터만 저장하므로 KV 캐시가 사용해야 할 메모리 감소
- 단점
  - 멀티 헤드 어텐션에 비해 성능 저하
  - 추가 학습을 기존 학습 데이터의 10%까지 사용해도 멀티 헤드 어텐션에 비해 큰 성능 차이

**그룹 쿼리 어텐션**: 멀티 헤드 어텐션보다는 적은, 멀티 쿼리 어텐션보다는 많은 키와 값 벡터 (둘의 절충안)
...
**비츠앤바이츠**: 양자화를 쉽게 사용할 수 있도록 제공하는 라이브러리

> 8비트 양자화 과정
1. 입력 행렬의 값 중 크기가 큰 이상치가 포함된 열은 별도로 분리하여 양자화 X
2. 정상 범위에 있는 열은 8비트 양자화 O
3. 1번과 2번을 각각의 가중치와 행렬 곱
4. 최종적으로 16비트 결과 산출

양자화에 별도의 시간이 소요되지 않으므로 모델을 불러 올 때 양자화 설정을 전달하여 바로 양자화 가능

### 7.2.2 GPTQ

**GPTQ**: 양자화 이전 모델과 양자화 이후 모델에 입력 행렬을 넣었을 때 오차가 가장 작아지도록 하는 양자화 방식

> GPTQ 양자화 과정
1. 양자화를 위한 작은 데이터셋 준비
2. 순차적으로 열 별로 양자화
3. 준비한 데이터를 입력하며 양자화 이전 모델의 결과와 최대한 근사하도록 양자화 할 열의 파라미터 업데이트

양자화 설정과 함께 데이터셋도 전달

매번 결과를 확인하여 순차적으로 양자화를 수행하므로 시간 소요

### 7.2.3 AWQ

**AWQ**: 특별히 중요한 파라미터의 정보를 유지한 채 양자화를 수행하는 방식

> 특정 파라미터의 중요도를 따지는 방법
- 모델 파라미터의 값이 크면 중요
- **입력 데이터의 활성화 값이 큰 채널의 파라미터는 중요**

> AWQ 양자화 과정
1. 중요도 상위 1%에 해당하는 모델 파라미터는 양자화 X
2. 나머지는 양자화 O

**모델 파라미터에 서로 다른 데이터 타입이 섞여 있으므로 연산 속도 저하, 효율성 감소**

> 해결책
- 1보다 크고 2보다 작거나 같은 스케일러를 중요한 파라미터에 곱하고 양자화

양자화에 많은 시간이 소요되지 않고 기존 모델의 성능을 거의 유지

## 7.3 지식 증류 활용하기

**지식 증류**: 성능 높은 선생 모델의 생성 결과를 활용하여 성능 낮은 학생 모델을 만드는 방법

> LLM에서 지식 증류 활용 사례
- 제퍼-7B-베타
  - 지시 데이터셋과 선호 데이터셋 구축에 레이블러가 아닌 GPT-4 사용
- 파이-1
  - 교육적 가치가 높은 데이터셋을 선택하고 해당 코드에 독스트링을 입력하고 코드 예제 데이터셋 구축에 GPT-3.5 사용
...
> 일반 배치의 문제점
- 생성이 종료된 특정 입력은 다른 데이터의 추론을 기다리느라 결과를 반환 불가
- 추론이 진행되며 생성이 종료되는 문장이 많아지므로 배치 크기가 작아져 GPU의 효율적인 사용을 방해

### 8.1.2 동적 배치

> **동적 배치**: 비슷한 시간대에 들어오는 요청을 하나의 배치로 묶어 배치 크기를 키우는 배치 방식
- 장점
  - 전체적인 처리량 증가
- 단점
  - 특정 요청에 대한 지연 시간 추가
  - 배치 크기가 작아져 GPU의 비효율적인 사용은 여전

### 8.1.3 연속 배치

**연속 배치**: 하나의 토큰 생성이 끝날 때마다 생성이 종료된 문장은 제거하고 새로운 문장 추가
...
**플래시어텐션**: 트램스포머가 더 긴 시퀀스를 처리하기 위해 개발된 기술

> 기존의 어텐션 연산
- 시퀀스 길이의 제곱에 비례하는 연산량
- 마스크, 소프트맥스, 드롭아웃 연산은 행렬곱에 비해 연산량은 작으나 데이터의 전달 속도가 느린 GPU의 고대역폭 메모리를 사용해서 소요 시간 증가

> 플래시어텐션의 장점
- 블록 단위로 어텐션 연산을 수행하여 어텐션 연산의 속도 증가
- 고대역폭 메모리가 아닌 데이터 전달 속도가 빠른 SRAM 사용하여 연산의 속도 증가

### 8.2.2 플래시어텐션 2

플래시어텐션 2: 플래시어텐션을 개선하여 2배 정도 속도 향상된 기술

> 플래시어텐션 2의 장점
- 비 행렬 곱셈 연산을 최대한 효율적으로 수행하여 연산의 속도 증가
- 시퀀스 길이 방향으로 여러 묶음으로 병렬화하여 스레드 블록 수를 늘림

### 8.2.3 상대적 위치 인코딩

**상대적 위치 인코딩**: 토큰과 토큰 사이의 상대적인 위치 정보를 추가하는 기술

> RoPE
- 각각의 토큰 임베딩을 토큰 위치에 따라 회전
- 토큰 사이의 위치 정보가 두 임베딩 사이의 각도로 반영

> ALiBi
- 어텐션 행렬에 오른쪽에서 왼쪽으로 갈수록 더 작은 값을 더하는 방식
- 간단한 인코딩 방식이므로 학습과 추론에 별도로 추가되는 처리 시간이 없음
...
> 커널 단위 연산의 문제점
- 연산 전후로 고대역폭 메모리에서 전달하는 작업과 같은 오버헤드가 발생
- 오버헤드로 인해 연산 시간 증가

> 커널 퓨전의 장점
- 반복적으로 수행하는 연산은 각 커널 단위로 묶어 오버헤드를 줄임

### 8.3.2 페이지어텐션

> KV 캐시의 문제점
- GPU 메모리의 약 30% 소모
- 최대 생성 토큰 수만큼 미리 메모리 확보하여 사용되지 않는 메모리 낭비

> 페이지어텐션의 장점
- 논리적 메모리와 물리적 메모리를 연결하는 **블록 테이블** 사용
- 물리적으로 연속된 메모리를 사용하지 않지만 논리적 메모리에서는 연속적
- 한 번에 최대 생성 토큰 수만큼 메모리를 확보하는 것이 아닌, 최대 블록 크기만큼 메모리 확보
- 최대 (블록 크기 - 1)만큼의 메모리만 낭비되므로 효율적인 GPU 활용

**참조 카운트**: 믈리적 블록을 공유하고 있는 논리적 블록 수
- 참조 카운트가 1보다 클 경우(= 동일한 토큰을 사용해야 하는 경우), 새로운 물리적 블록에 기존 토큰 복사
- 동일한 토큰은 최대한 메모리를 공유하고, 새롭게 생성하는 토큰만 분리하여 저장

### 8.3.3 추측 디코딩

**추측 디코딩**: 쉬운 단어는 더 작고 효율적인 모델이 예측, 어려운 단어는 더 크고 성능이 좋은 모델이 예측

> 추측 디코딩의 과정
1. 작은 드래프트 모델이 빠르게 임의의 토큰 생성
2. 큰 타깃 모델이, 드래프트 모델이 생성한 토큰이 자신이 추론했다면 생성했을 결과와 비교하여 승인 혹은 거절
3. 거절한 토큰은 없애고 타깃 모델이 토큰 하나 추가

> 추측 디코딩의 장점
- 드래프트 모델의 추론 시간은 타깃 모델의 추론 시간에 비해 매우 빠름
- 타깃 모델 한 번의 추론 시간과 유사한 시간 동안 여러 개의 토큰 생성 가능

## 8.4 실습: LLM 서빙 프레임워크

### 8.4.1 오프라인 서빙

오프라인 서빙: 정해진 입력 데이터에 대해 배치 추론을 수행

- 모델에 입력할 데이터가 이미 정해져 있으므로 배치 처리를 통해 처리량을 높일 수 있음

### 8.4.2 온라인 서빙

온라인 서빙: 사용자의 요청이 올 때 모델 추론을 수행

# LLM을 활용한 실전 AI 애플리케이션 개발 | 3부 LLM을 활용한 실전 애플리케이션 개발

# 9. LLM 애플리케이션 개발하기

## 9.1 검색 증강 생성(RAG)

**RAG**: 답변에 필요한 충분한 정보와 맥락을 제공하고 답변하도록 하는 방법

> RAG의 과정
- 검색할 데이터를 벡터 데이터베이스에 저장하는 과정
- 사용자의 요청에 관련된 정보를 벡터 데이터베이스에서 검색한 후, 사용자의 요청과 결합에 프롬프트를 완성하는 과정

### 9.1.1 데이터 저장

데이터 소스를 임베딩 모델을 사용해 벡터 데이터베이스에 저장

벡터 데이터베이스: 임베딩 벡터의 저장소이며 입력한 벡터와 유사한 벡터를 찾는 기능 제공

### 9.1.2 프롬프트에 검색 데이터 통합
