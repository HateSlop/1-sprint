...
교차 인코더를 활용해 필터링 된 문장 사이의 유사도를 계산하고 순위를 변경하는것을 **순위 재정렬**이라고 함.
...
학습은 **4 에포크** 동안 진행(전체 학습 데이터를 4번 반복해서 학습), 기본 언어 모델은 **klue-roberta**를 사용, 학습 데이터를 문장 임베딩으로 변환하고 두 문장 사이의 **코사인 유사도**와 정답 유사도를 비교해 학습을 수행.
...
데이터를 적절한 임베딩 모델을 통해 임베딩 벡터로 변환했다면 이제 벡터 데이터베이스에 저장하고 임베딩 벡터의 거리 계산을 통해 유사하거나 관련이 깊은 데이터를 검색할 수 있다. 벡터 데이터 베이스는 벡터 사이의 거리 계산에 특화된 데이터베이스이다. 

---

### 12.1.1 딥러닝과 벡터 데이터베이스

벡터 데이터베이스의 등장은 딥러닝의 발전과 관련이 깊다. 딮러닝은 비정형 데이터를 잘 처리한다는 강점이 있고 머신러닝과의 가장 큰 차이는 사람이 집접 데이터의 특징을 정의하는지 여부다. 

* 표현 학습: 딥러닝에서 충분한 데이터로 모델이 알아서 특징을 뽑는 과정을 학습하는 것.

* 벡터 데이터베이스를 활용하기 위한 세 단계    
...
벡터 임베딩을 저장하고 검색하는 기능을 구현하려고 할 때 만나는 소프트웨어는 크게 세 가지   
![alt text](image-56.png)   

* 벡터 라이브러리와 벡터 데이터베이스의 차이...?   
벡터 데이터베이스는 저장과 검색 기능 외에도 다음과 같은 기능을 제공   
...
* KNN 검색: 검색하려는 벡터와 가장 가까운 K개의 이웃 벡터를 찾는 검색 방식. 직관적이고 모든 데이터를 조사하기 때문에 정확하지만 모든 벡터를 조사해 연산량이 데이터 수에 비례하게 늘어나 속도가 느려지기 때문에 확장성이 떨어진다는 한계가 있다.

먼저 벡터 검색을 하기 위해 인덱스(테이블)를 만들어야 함.   
인덱스에 벡터를 저장하는 과정을 '색인한다'라고 표현    
색인 단계에서는 인덱스의 메모리 사용량과 색인 시간이 중요.   
검색 단계 에서는 검색 시간과 재현율이 중요 

---

### 12.2.2 ANN 검색이란 

* ANN 검색: 대용량 데이터셋에서 주어진 쿼리 항목과 가장 유사한 항목을 효율적으로 찾는 데 사용되는 기술. 약간의 정확도를 희생하는 대신 훨씬 더 빠른 검색 속도를 제공   

* 대표적인 ANN 알고리즘    

    * IVF 알고리즘: 인덱싱 시점에 데이터를 가져와 중심정(유사한 벡터들의 클러스터)를 형성 -> 해당 중심점 내에서 가장 가까운 데이터 포인트를 찾음으로써 가장 가까운 데이터 포인터를 찾는다. 이렇게 몇 개의 클러스터만 검색하는 것이 IVF의 전략.    

    * HNSW 알고리즘: 그래프 기반 인덱싱 구조, 상위 계층에는 연결이 적고 하위 계층에는 연결이 밀집된 다층 그래프를 구축. 최상위 계층에는 연결이 적기 때문에 데이터 사이의 거리가 먼데, 한 번에 먼 거리를 이동할 수 있기 때문에 그래프를 빠르게 탐색할 수 있게됨. 각 하위 층에는 더 많은 연결이 추가되기 때문에 더 꼼꼼한 탐색이 가능한다. 검색은 최상위 계층에서 시작한 다음 검색을 구체화하기 위해 아래 계층으로 진행한다. 

---

### 12.2.3 탐색 가능한 작은 세계(NSW)

HSNW는 탐색 가능한 작은 세계(NSW)와 계층 구조(Hierarchy)를 결합한 것. 
NSW는 **적당히 랜덤하게** 연결된 그래프 상태. 규칙적인 연결에서 일부 연결만 랜덤하게 바꾼 형태로, 규칙적인 연결을 통해 정확한 탐색이 가능하면서도 랜덤한 성질을 통해 빠른 탐색이 가능해진다는 특성을 가짐. 하지만 지역 최솟값 문제 발생     
...
HNSW는 여러 계층에 그래프를 배치하는 방식으로 벡터를 저장한다.    
높은 층에 있는 데이터는 그보다 낮은 층에 항상 존재한다.    
그러면 높은 층에 벡터를 저장할지 말지 어떻게 결정할까?     
![alt text](image-58.png)   
실제로는 복잡하지만 이런 방식이다. 이렇게 확률을 이용해 높은 층으로 갈수록 데이터가 적어지는 구조를 만든다. 그러면 높은 층에 있는 벡터는 NSW에서 일부 랜덤 연결이 탐색 단계를 줄인 것처럼 탐색 단계를 줄여준다. 그러면서도 낮은 층에는 모든 벡터가 저장돼 있기 때문에 지역 최솟값에 빠질 위험을 줄일 수 있다. 
...
색인 과정에서 가장 가까운 M개를 선택하기 위해 저장하는 후보의 (ef_construction),    
검색 과정에서 가장 가까운 K개를 선택할 때 저장하는 후보의 수(ef_search)   
세 가지 파라미터로 메모리 사용량, 색인  시간, 재현율, 검색 시간이 달라진다.    
...
* MLops는 데브옵스의 개념을 머신러닝과 데이터 과학 분야로 확장한 방법론이다.   
기존의 데브옵스가 소프트웨어 개발과 운영의 협업과 자동화에 초점을 맞췄다면, MLOps는 여기에 데이터와 머신러닝 모델이라는 두 가지 핵심 요소를 더해 확장한 것이다. 

![alt text](image-60.png)   
MLOps에서는 모델의 재현성을 보장하는 것이 매우 중요하다.(이전에 수행된 ML 워크플로를 그대로 반복했을 때 동일한 모델을 얻을 수 있는지 여부)   

MLOps에서는 모델 학습을 자동으로 트리거하여 새로운 데이터로 지속적으로 모델을 업데이트한다. 모델 성능 저하 시 자동으로 재학습과 배포가 이뤄지도록 함으로써 모델이 항상 최적의 상태를 유지하도록 만든다.   

---

### 13.1.1 데이터 관리   

포함시킬 데이터의 범위를 선택하고 어떤 전처리 방식을 포함시킬지, 특성 공학을 통해 어떤 특성을 추가할지에 따라 학습 데이터셋이 달라진다.    
모델 학습 결과를 재현하기 위해서는 데이터셋의 버전을 관리하고 어떤 학습 데이터셋으로 모델을 학습시켰는지 기록해야 한다. 이를 위해 DVC와 같은 도구를 사용할 수 있다.   

--- 

### 13.1.2 실험 관리   

모델마다 성능에 영향을 주는 다양한 하이퍼파라미터가 있는데 어떤 하이퍼파라미터가 최적인지 탐색하는 과정이 필요.    
실험 관리(추적)도구를 사용하며 대표적으로는 MLflow, W&B가 있다.    
...
MLOps에서 모델 저장소는 머신러닝 모델을 체계적으로 관리하고 버전 제어하는 데 필수적인 요소.   
머신러닝 모델 개발 과정에서 여러 머신러닝 파이프라인에서 다양한 실험을 통해 여러 버전의 모델이 생성되는데, 모델 저장소를 통해 다양한 모델을 하나로 통합해서 관리 할 수 있음.    

---

### 13.1.4 모델 모니터링 

머신러닝 모델은 학습 데이터를 통해 학습한 패턴을 바탕으로 예측을 수행하기 때문에 정상적으로 요청에 응답하고 있다고 하더라도 엉뚱한 값을 반환한 것은 아닌지 확인해야 한다. 이를 위해 모니터링을 수행한다.   
성능뿐만 아니라 머신러닝 시스템의 기본적인 지표를 기록해 시스템에 문제가 없는지, 추가적인 리소스의 확보가 필요한지, 리소스에 낭비가 없는지 지속적인 확인이 필요하다. 
...
LLMOps에서 다루는 LLM은 기존 머신러닝 모델에 비해 훨씬 크고 OpenAI나 구글 같은 일부 기업이 API 기반으로 상업용 모델을 제공한다는 점이 가장 큰 차이점이다. 또 기존 머신러닝 모델이 주로 분류나 회귀처럼 평가 지표가 명확한 작업에 주로 사용됐다면 LLM은 생성 작업을 수행하므로 결과물을 정량적으로 평가하기 어려워졌다.   

![alt text](image-61.png)   
대규모 언어 모델은 API를 통해 사용할 수 있는 상업용 모델과 직접 모델을 학습 시키고 실행해 활용할 수 있는 오픈소스 모델로 양분됐다.   

---

### 13.2.2 모델 최적화 방법의 변화   

LLM을 사용 사례에 맞게 최적화 하는 방법은 크게 사전 학습, 미세 조정, 프롬프트 엔지니어링, 검색 증강 생성(RAG) 네 가지로 나눌 수 있다.  
...
* BLEU: 번역 작업을 평가할 때 사용, 기계 번역 결과와 사람이 번역한 결과의 유사도를 측정하여 번역 성능을 평가. 

* ROUGE: 요약이나 번역 등 자연어 생성 모델의 성능 평가에 사용, 모델이 생성한 요약문과 사람이 작성한 참조 요약문 사이의 n그램 중복도를 재현율 관점에서 측정  

* 펄플렉시티: 모델이 새로운 단어를 생성할 때의 불확실성을 수치화한 것, 값이 낮을수록 모델의 예측 성능이 우수하다는 의미 

세 가지 정량 지표 모두 빠르게 언어 모델의 성능을 평가할 수 있다는 장점이 있지만 문장의 의미, 문법, 유창성 등 질적인 측면의 평가에는 한계가 있고 실제 사람의 주관적 판단과 불일치하는 경우가 많다. 
...
ARC, HellaSwag, MMLU, TruthfulQA 데이터셋 등..

---

### 13.3.3 사람이 직접 평가하는 방식

사람이 직접 평가하는 방식은 언어의 유창성과 같이 정량적인 지표로 평가하기 어려운 사항을 평가할 수 있다는 장점이 있지만 시간이 오래 걸리고 비용이 많이 든다는 단점. 
...
LLM이 더 많은 종류의 일을 처리하기 위해서는 더 많은 데이터 유형을 활용할 수 있어야 함. 이를 위해 멀티 모달 연구가 활발히 진행되고 있음.   
멀티 모달 모델을 LLM과 유사하게 LMM(Large Multi-modal Model)이라는 용어로 부름.   
이번 장에서는 이미지 데이터를 LLM에 활용하기 위한 흐름을 중심으로 멀티 모달 모델에 대해 알아본다.   

## 14.1 멀티 모달 LLM이란 
 
멀티 모달 LLM: 텍스트뿐만 아니라 이미지, 비디오, 오디오, 3D 등 다양한 형식의 데이터를 이해하고 생성할 수 있는 LLM.   

### 14.1.1 멀티 모달 LLM의 구성요소  

멀티 모달 LLM은 다섯 가지 구성요소로 이뤄진다.   
그 중 LLM이 가장 핵심적인 역할을 하는데, 이미지 형석의 데이터를 **모달리티 인코더**와 **입력 프로젝터**를 통해 텍스트로 변환해 LLM에 입력한다.   
LLM의 출력은 기본적으로 텍스트인데, **출력 프로젝터**를 통해 이미지 형태의 데이터 출력이 필요한지 판단하고 **모달리티 생성기**를 통해 특정 데이터 형식의 출력을 생성한다.   

![alt text](image-64.png)   

> **얼음 아이콘**이 있는 3개의 구성요소는 학습에 많은 데이터와 연산량이 필요하기 때문에 멀티 모달 LLM 학습 과정에서 파라미터를 **업데이트하지 않고** 사전 학습된 모델을 그대로 사용한다. **불꽃 아이콘**이 있는 입력 프로젝터와 출력 프로젝터는 모달리티 인코더와 LLM 백본, 모달리티 생성기를 **연결하는 구성요소**로 멀티 모달 LLM 학습 과정에서 **파라미터를 업데이트함**으로써 멀티 모달 이해와 생성 성능을 높인다. 

* 비전 트랜스포머 : 텍스트를 처리하기 위해 개발된 트랜스포머 아키텍처를 이미지에 적용한 모델, 이미지를 패치(patch)단위로 자른 후 마치 텍스트에서 단어를 처리하는 것과 같이 일렬로 나열해 입력해 처리한다. 이미지 패치의 경우 사전을 구축하기 어렵기 때문에 선형 변환을 통해 이미지 임베딩 벡터로 변환함으로써 토큰 임베딩과 유사하게 만든다.   
...
멀티 모달 LLM의 학습 과정은 LLM과 마찬ㄷ가지로 사전 학습과 지시 데이터셋을 활용한 지시 학습으로 나뉜다.   

사전 학습 단계에서 LLM은 이미지-텍스트 쌍과 같은 서로 다른 모달리티 표현을 이해하고 그 관계를 연결하는 방법을 학습한다. 이 단계에서는 전체적인 이해력을 습득하고 높이는 데 집중한다.   
사전 학습이 끝난 후, 멀티 모달 지시 튜닝 단계를 진행한다. 이때 멀티 모달 LLM을 소규모 멀티 모달 지시 데이터셋으로 미세 조정한다.   

---

## 14.2 이미지와 텍스트를 연결하는 모델: CLIP    

이번 절에서는 대표적인 이미지 인코더인 OpenAI의 CLIP모델을 살펴본다. 

### 14.2.1 CLIP 모델이란 

CLIP: 텍스트 데이터와 이미지 데이터의 관계를 계산할 수 있도록 텍스트 모델과 미미지 모델을 함께 학습시킨 모델로, 2021년 OpenAI에서 개발했다.   

근데 텍스트와 이미지 두 가지 서로 다른 데이터 형식의 관계는 어떻게 학습시킬 수 있을까?????

---

### 14.2.2 CLIP 모델의 학습 방법 

CLIP 모델을 학습 시킬때에도 문장 임베딩 모델과 마찬가지로 서로 관련이 있는 이미지와 텍스트 쌍을 활용한다.   
이때 이미지-텍스트 쌍의 데이터란 이미지와 이미지에 대한 설명이 대응된 데이터를 말한다.   

* 학습 방법   
![alt text](image-65.png)

CLIP은 대조 학습을 통해 모델을 학습시킨다.   
이미지와 텍스트를 각각 이미지 인코딩 모델과 텍스트 인코딩 모델에 얺어 임베딩으로 만든다. 서로 매칭되는 쌍의 유사도는 커지고 서로 매칭되지 않는 쌍의 유사도는 작아지도록 학습시킨다. 이때 텍스트 인코더로는 트랜스포머 모델을 사용하고 이미지 인코더로는 앞서 살펴본 ViT나 ResNet같은 이미지 모델을 사용한다.  

---

### 14.2.3 CLIP 모델의 활용과 뛰어난 성능   

학습을 마친 CLIP 모델은 제로샷 추론을 수행할 수 있다.    
...
디퓨전 모델은 확산 현상에서 영감을 받아 만들어진 생성 모델이다.   
디퓨전 모델은 이미지에서 어떤 부분이 노이즈인지 예측하는 방식으로 학습하는데, 그 능력을 사용해 완전한 노이즈 상태의 이미지에서 노이즈를 예측하고 예측된 노이즈를 제거하면서 점차 완전한 노이즈에서 의미가 있는 이미지를 생성한다.   

**U-Net**이라는 인코더 디코더 모델을 많이 사용해 노이즈를 예측.   
인코더 디코더란, 입력 데이터의 차원을 낮추는 인코딩 단계와 차원을 높이는 디코딩 단계를 통해 데이터의 의미를 압축하기 위해 사용되는 모델 구조.   

![alt text](image-66.png)   

U-Net은 인코더-디코더 구조를 변행해 인코딩 단계의 고차원 정보를 디코딩 단계에서도 활용함으로써 이미지의 위치 정보가 손실되는 것을 막을 수 있다는 장점이 있다.   

그런데 완전한 노이즈 상태라면 노이즈를 제거했을 때 어떤 이미지가 생성될지 어떻게 알 수 있을까?   

![alt text](image-67.png)

위 그림과 가이 사람이 원하는 이미지를 생성하기 위해서는 디퓨전 모델에 노이즈를 넣어주면서 원하는 겨로가물의 형태를 텍스트 임베딩으로 변환해 디퓨전 모델에 함께 입력으로 넣어준다. 이때 넣어주는 추가 정보는 텍스트 임베딩이 될 수도 있고 이미지 임베딩이 될 수도 있다.  

---

### 14.3.2 DALL-E 모델  

DALL-E 모델은 두 단계를 거쳐 이미지를 생성한다.   
![alt text](image-68.png)

윗 부분은 매칭되는 텍스트 임베딩과 이미지 임베딩의 거리를 가깝게 만드는 CLIP 모델의 학습 방법을 나타낸다. 아랫부분은 DALL-E 2의 모델 구조를 보여준다. 먼저, CLIP의 텍스트 인코더를 사용해 입력한 텍스트를 텍스트 임베딩으로 만든다. 그리고 프라이어 모델을 통해 CLIP 이미지 임베딩을 만들고 디코더를 사용해 이미지를 생성한다.    

이때 프라이어는 텍스트 임베딩을 입력으로 받아 이미지 임베딩을 예측하는 디퓨전 모델이다.   
...
입력 이미지를 CLIP의 이미지 인코더를 통해 이미지 임베딩으로 만들고 간단한 선형 층을 통과해 LLM에 입력할 임베딩 토큰으로 만든다. 텍스트 지시사항은 토큰 임베딩으로 변환해 함께 입력으로 넣고 결과를 생성한다.   
...
LLaVA-1.5는 비교 대상이 된 다른 모델에 비해 훨씬 적은 데이터로 학습했음에도 더 뛰어난 성능을 보였다. 이때 원래 모델과는 다르게 이미지 인코더를 ViT-L/336px로 바꾸고 한 층의 선형 층으로 이미지 임베딩을 토큰 임베딩으로 변환하던 구조를 2층의 MLP로 변경하는 간단한 수정만 했다.   

---

### 14.4.4 LLaVA NeXT  

그 LLaVA 1.5를 더 발전시킨 모델로 다음과 같은 사항이 변경되었다.    
![alt text](image-71.png)

가히 오픈소스 모델 중 최고 성능을 달성했고 상업용 모델인 구글의 것들 보다 대부분의 벤치마크에서 높은 성능을 보였다. 
...
## 15.1 에이전트란
### 15.1.1 에이전트의 구성요소    
* 에이전트: 알아서 생각하고 행동하는 시스템, 주변 환경을 감각을 통해 인식하고 의사결정을 내려 행동하는 인공적인 개체   
* 에이전트의 세 가지 구성요소 : 감각, 두뇌, 행동    
### 15.1.2 에이전트의 두뇌   
...
* AutoGPT의 수행 방법    
1. 에이전트의 이름과 역할 부여   
2. 구체적으로 역할을 정의할 경우 그렇지 않았을 때보다 역할과 관련된 기능을 더 잘 수행하는 것으로 알려져있다. 다음으로 목표를 정해준다.   
3. 에이전트가 작동하는 동안 지켜야 하는 제약사항이 있다면 전달한다.   
-> 모든 과정을 스스로 처리하기 때문에 매우 편리, 다양한 작업에 사용 가능     
### 15.2.2 사용자와 에이전트의 상호작용    
특정 도메인에서 LLM의 전문성이 사람에 비해 떨어지는 경우 사용자가 문제 해결 과정에 직접 개입하는 것이 중요하다. 사용자가 명확한 명령과 피드백으로 에이전트의 작업 방향을 설정하고 에이전트는 작은 작업을 자동으로 수행하서 사용자가 빠르고 편하게 작업을 진행하도록 지원할 수 있다.   
### 15.2.3 멀티 에이전트   
* 멀티 에이전트 방식: 여러 에이전트의 협렵을 통해 문제를 해결, 각 에이전트마다 서로 다른 프로필을 주고 작업을 수행.   
여러 에이전트가 대화를 나누며 문제를 해결할 때는 모두 함께 대화를 진행하는 수평형 대화와 대화를 주도하는 매니저가 상황에 따라 작업할 에이전트를 선택하고 대화가 진행되는 위계형 대화를 활용할 수 있음.   
...
1. 주관적인 방식   
2. 테스트 에이터로 평가하는 방식    
이때 주관적인 방식은 다시 두 가지로 나뉨.   
![alt text](image-73.png)   
* 1-1 에이전트의 작업 결과를 사람이 직접 평가하는 방식   
* 1-2 사람의 결과와 비교해서 에이전트가 생성한 것이 사람이 생성한 것을 구분할 수 있는지 확인하는 방식    
...
## 16.2 SSM   
맘바는 SSM 모델이다.    
* SSM: 내부 상태를 가지고 시간에 따라 달라지는 시스템을 해석하기 위해 사용하는 모델링 방법,내부 상태와 출력이 시간에 따라 달라지는 과정을 수식화한다.    
### 16.2.1 S4   
대표적인 SSM모델 중 하나인 S4: SSM모델의 계산 효율성을 높여 쉽게 모델을 학습시킬 수 있도록 함.   
## 16.3 선택 메커니즘   
* 선택 메커니즘: 텍스트를 읽고 이해할 때 핵심적인 부분은 텍스트의 맥락을 고려해 새로운 입력을 해석하는 부분   
RNN을 발전시킨 LSTM모델은 맥락을 더 효율적으로 압축하기 위해 압력을 얼마나 반영할지, 기존 상태를 얼마나 망각할지 결정하는 게이트를 추가   
![alt text](image-75.png)   
## 16.4 맘바   
### 16.4.1 맘바의 성능    
![alt text](image-76.png)   
맘바 아키텍처는 높은 성능과 빠른 학습, 추론 속도를 갖춰 연산이 부담이 큰 트랜스포머 아키텍처의 강력한 대안으로 주목받고 있다.   
### 16.4.2 기존 아키텍처와의 비교    
가장 중요한 기준은 **맥락을 얼마나 압축하느냐** 이다.   
